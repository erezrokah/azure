...
FROM azure_container_registries r
         ...
         LEFT JOIN azure_network_virtual_network_subnets anvns ON acrnrsvnr.virtual_network_resource_id = anvns.id
WHERE r.network_rule_set_default_action IS DISTINCT FROM 'Deny'
   OR acrnrsvnr.virtual_network_resource_id IS NULL
